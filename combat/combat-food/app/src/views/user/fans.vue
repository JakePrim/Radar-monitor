<template>
  <div class="fans">
    ...
    <ul class="fans clearfix">
      <router-link :to="{name:'space',query:{userId:item.userId}}" tag="li" v-for="item of info" :key="item._id">
        <a href="" class="img">
        ...
        <div class="c">
          ...
          <em class="info"><span>简介</span>{{ (item.sign || item.sign === '') ? `这个人太懒啦！还没有介绍自己` : item.sign }}</em>
        </div>
      </router-link>
    </ul>
  </div>
