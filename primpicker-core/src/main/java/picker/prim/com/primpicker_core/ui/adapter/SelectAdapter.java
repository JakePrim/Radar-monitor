...
public class SelectAdapter extends CursorAdapter<RecyclerView.ViewHolder> implements MediaGridView.OnMediaGridItemClick {
    ...
    @Override
    public void clickItem(ImageView imageView, PrimCheckBox checkBox, MediaItem item, RecyclerView.ViewHolder viewHolder) {
        if (SelectSpec.getInstance().isPerViewEnable && SelectSpec.getInstance().isClickItemPerOrSelect) {//预览item
    ...
    @Override
    public void clickSelectItem(boolean isCheck, PrimCheckBox checkBox, MediaItem item, RecyclerView.ViewHolder viewHolder) {
        int checkNumOf = selectItemCollection.checkNumOf(item);
        if (checkNumOf == Integer.MAX_VALUE) {
            selectItemCollection.add(item);
            notifyCheckStateChanged();
